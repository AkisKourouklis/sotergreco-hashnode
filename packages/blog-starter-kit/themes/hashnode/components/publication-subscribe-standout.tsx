    ...
    <>
      <div className="my-10 mt-20 flex w-full flex-col items-center pb-10 md:px-5">
        ...
        <p className="mb-5 text-center text-lg text-slate-700 dark:text-slate-300 md:w-2/3 md:text-xl">
          Read articles from {<strong>{publication!.title}</strong> || 'this blog'} directly inside your inbox.
          Subscribe
          to the newsletter, and don&apos;t miss out.
        </p>
        {!state.subscribed && (
          <div
            className="flex flex-row overflow-hidden rounded-lg border border-slate-800 dark:border-slate-200 md:w-2/3">
            <input
              type="email"
              ref={email}
        ...
        {state.subscribed && (
          <div
            className="flex flex-col items-center overflow-hidden rounded-lg border border-green-600 bg-green-50 p-5 text-center text-slate-900 dark:border-green-400 dark:bg-green-900 dark:text-white md:w-2/3">
